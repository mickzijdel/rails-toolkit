...
- **rails-search** — Postgres-first in-app search: `pg_search` full-text (`tsearch`) with weighted columns and ranking, `pg_trgm` trigram fuzzy matching for autocomplete/typo tolerance, multi-model search via `PgSearch.multisearch`, tenant-scoped and N+1-safe queries, background reindexing, an escalation ladder to Meilisearch/Elasticsearch/Algolia, and testing search behavior against real Postgres.
